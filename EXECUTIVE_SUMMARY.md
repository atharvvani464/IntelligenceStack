...
4. **Contextual Retrieval (Mosaic AI Vector Search):** Unstructured enterprise knowledge — runbooks, playbooks, and policies — is synchronized into a Vector Index, allowing the Agent to perform secure Retrieval-Augmented Generation (RAG) without that knowledge leaving the perimeter. **This pillar is realized in code in the sandbox** (`src/lakehouse/knowledge_engine.py`): a real vector index is built over the governed corpus and ranked by cosine similarity computed inside the engine. Retrieval is subject to the same governance boundary as every other capability, every recommendation is attributed to the document it came from, and the agent **abstains** when a question is not covered rather than citing a weak match — because a confident but spurious citation is worse than an honest "no". The result is a *hybrid* agent that can combine a live metric with the documented procedure for acting on it, in one governed answer.
...
- The local **TF-IDF vector index** is replaced by **Mosaic AI Vector Search** with dense neural embeddings (`databricks-bge-large-en`) — same storage layout, same retrieval contract, same citations.
